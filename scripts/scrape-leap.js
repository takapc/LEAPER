...
import { normalizeMeanings } from '../src/utils/meanings.js'
 ...
 * @returns {Array<{id: number, word: string, meanings: import('../src/utils/meanings.js').Meaning[]}>}
 */
  ...
  rows.forEach((row) => {
    ...
    if (cells.length >= 3) {
      ...
      if (!isNaN(id) && id > 0 && word && meaning) {
        words.push({ id, word, meanings: normalizeMeanings(meaning) })
      }
    }
  })
